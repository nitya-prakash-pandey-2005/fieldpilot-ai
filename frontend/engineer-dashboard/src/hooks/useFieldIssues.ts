...
export function useFieldIssues(projectId: string = 'default-project') {
  ...
  const fetchIssues = useCallback(async () => {
    try {
      setIsLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/projects/${projectId}/issues`);
  ...
  useEffect(() => {
    const eventSource = new EventSource(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/projects/${projectId}/issues/stream`);
  ...
  const resolveIssue = async (issueId: string, note: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/issues/${issueId}/resolve`, {
        method: 'POST',
  ...
  const escalateIssue = async (issueId: string, role: string, note: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/v1/issues/${issueId}/escalate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
